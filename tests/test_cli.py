...
def test_build_parser_accepts_profile_options() -> None:
    ingest_args = build_parser().parse_args(["ingest", "data/raw", "--profile", "technical"])
    retrieve_args = build_parser().parse_args(["retrieve", "cats", "--profile", "recipes"])
    ask_args = build_parser().parse_args(["ask", "cats", "--profile", "technical"])
    profiles_args = build_parser().parse_args(["profiles"])

    assert ingest_args.profile == "technical"
    assert retrieve_args.profile == "recipes"
    assert ask_args.profile == "technical"
...
def test_build_parser_accepts_index_management_commands() -> None:
    delete_args = build_parser().parse_args(["delete-source", "docs/example.pdf"])
    reset_args = build_parser().parse_args(["reset-index", "--yes"])

    assert delete_args.source == Path("docs/example.pdf")
...
def test_format_profiles_includes_profile_details() -> None:
    ...
    assert "- technical | style technical | chunk 800/100 | paths data/raw/tech | Tech docs" in output
...
def test_build_parser_accepts_summarize_command() -> None:
    args = build_parser().parse_args(
        ["summarize", "README.md", "--question", "main idea", "--summary-language", "deu"]
    )

    assert args.source == Path("README.md")
    assert args.question == "main idea"
    assert args.summary_language == "deu"
